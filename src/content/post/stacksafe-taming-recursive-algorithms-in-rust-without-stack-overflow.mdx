...
Recursive algorithms are elegant and intuitive, but they come with a fundamental limitation: stack overflow. Consider these common scenarios:

```rust
// Deep tree traversal - will crash on large trees
fn tree_sum(node: &TreeNode) -> i32 {
    match node {
        TreeNode::Leaf(value) => *value,
        TreeNode::Branch(left, right) => tree_sum(left) + tree_sum(right),
    }
}
...
```rust
fn tree_sum_iterative(root: &TreeNode) -> i32 {
    let mut stack = vec![root];
    let mut sum = 0;
    while let Some(node) = stack.pop() {
        match node {
            TreeNode::Leaf(value) => sum += value,
...
This approach works for simple cases but becomes extremely complex or impossible when:
...
```rust
#[derive(Clone, Debug)]  // These will still stack overflow!
enum Tree {
    Leaf(i32),
...
let cloned = deep_tree.clone();  // Stack overflow here
println!("{:?}", cloned);        // Or here
```
