...
The `anyhow` documentation encourages using `.context()` to add information. But `.context()` is optional--the type system doesn't require it. Every `?` compiles just fine without context, making it trivially easy to skip. "I'll add context later" is the easiest lie to tell yourself. Later means never--until 3am when production is on fire.
