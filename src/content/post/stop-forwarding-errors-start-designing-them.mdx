...
As noted in a [detailed analysis of error handling in RisingWave](https://bugenzhao.com/2024/04/24/error-handling-1/), a large Rust project:
...
Rust's `std::error::Error` trait makes a fundamental assumption: errors form a chain. Each error has an optional `source()` that points to the underlying cause.

This sounds elegant. In practice, it's often useless.

Many errors don't naturally form chains. A validation error with multiple field failures, a timeout with partial results—these don't fit the single-chain model.
...
**Capturing backtraces is expensive.** [One developer reported](https://github.com/rust-lang-nursery/error-chain/issues/129) their web app took a third of a second just to construct an error, making it vulnerable to DoS attacks. The standard library documentation acknowledges: "Capturing a backtrace can be a quite expensive runtime operation."
...
**Dynamic typing**: You request an opaque `TypeId` and hope something provides it. You might get a `u16` status code... or you might not. The meaning is implicit, undocumented, and varies by error type.

**Complexity**: The API is subtle enough that [LLVM struggles to optimize multiple provide calls](https://github.com/rust-lang/rust/issues/96024).
...
This looks reasonable. But notice how it categorizes errors: by *origin*, not by *what the caller can do about it*.
...
The anyhow documentation encourages using `.context()` to add information. But when you're racing against a deadline, "I'll add context later" is the easiest lie to tell yourself. Later means never—until 3am when production is on fire.
...
This is the fundamental disconnect in how we think about error handling. We focus on *propagating* errors correctly, on making the types line up, on satisfying the compiler. But we forget that errors are messages—messages that will eventually be read by either a machine trying to recover, or a human trying to debug.
...
It's a nice, simple rule. It's also not quite right. As [Luca Palmieri](https://lpalmieri.com/posts/error-handling-rust/): "It is not the right framing. You need to reason about intent."
...
The [exn](https://github.com/aspect-build/exn) library (841 lines of Rust, zero dependencies) demonstrates one approach: errors form a *tree* of frames, each automatically capturing its source location via `#[track_caller]`. Unlike linear error chains, trees can represent multiple causes—useful when parallel operations fail or validation produces multiple errors.
...
**Enforce context at library / module boundaries.** This is where exn differs critically from anyhow. With anyhow, every error is erased to `anyhow::Error`, so you can always just `?` and move on — the type system won't stop you. The context methods exist, but nothing *forces* you to use them.
...
Here's what this looks like in practice (from a production codebase):
...
fn find_error<T>(exn: &Exn<impl Error>) -> Option<&T> {
    fn walk(frame: &Frame) -> Option<&T> {
        if let Some(e) = frame.as_any().downcast_ref::<StorageError>() {
            return Some(e);
        }
